...
c
     ...
     & 	,stdsal,equatn,epsil,brntop,brnbot,itest,jtest
      USE HYCOM_ARRAYS,  only : temp,saln,dp,odhsi,surflx,p,pbot,salflx
     &  ,brnflx,sqiflx,osalt,diag1,diag2,diag3,diag4
...
c     pump=i.gt.equatn		! activate pump in southern hemisphere
c
...
css   tmxl=temp(i,j,k1n)+surflx(i,j)*delt1*g/(spcifh*dpth)
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
c
      if (tmxl.lt.tmelt) then
        amount=(tmelt-tmxl)*spcifh*dpth/g	! > 0 J/m^2
        surflx(i,j)=surflx(i,j)+amount/delt1	! > 0 W/m^2
c --- 'odhsi' is thermal energy per atmo time step transmitted to the ocean
        odhsi(i,j)=odhsi(i,j)-amount		! < 0 J/m^2
...
c --- a newly formed amount of ice 'dhmsi' (kg/m^2) sequesters the salt amount
...
c --- (1-fsss)*dmsi*stdsal/dt representing brine ejected during ice formation.
c --- sign convention: ice mass gain -> dmsi>0
c --- sign convention: ice salt gain -> dssi>0
c --- sign convention: ocn salt gain -> brnflx>0

        brnflx(i,j)=amount*(1.-fsss)*stdsal/
     .    ((tmelt*spcifh-Ei(tmelt,fsss*saln(i,j,k1n)))*delt1)	! >0  g/m^2/sec
        sqiflx(i,j)=-amount*   fsss *stdsal/
     .    ((tmelt*spcifh-Ei(tmelt,fsss*saln(i,j,k1n)))*delt1)	! <0  g/m^2/sec
        if (tmxl.lt.tmelt-1. .or. vrbos)
     .   print 100, nstep,i,j,' (enloan) -borrow- T=',temp(i,j,k1n),
     .     ' W/m2',amount/delt1,' brine',brnflx(i,j)*1.e6,' sequst',
     .     sqiflx(i,j)*1.e6
...
c --- add diagnostics
c     if (i < equatn) then
c       diag1(i,j)=diag1(i,j)+amount*fsss*stdsal/    ! g/m2/s
c    .    ((tmelt*spcifh-Ei(tmelt,fsss*saln(i,j,k1n)))*delt1)
c       diag2(i,j)=diag2(i,j)+osalt(i,j)*1.e3        ! g/m2/s
c     else
c       diag3(i,j)=diag3(i,j)+amount*fsss*stdsal/    ! g/m2/s
c    .    ((tmelt*spcifh-Ei(tmelt,fsss*saln(i,j,k1n)))*delt1)
c       diag4(i,j)=diag4(i,j)+osalt(i,j)*1.e3        ! g/m2/s
c     end if
c
      if (vrbos) write (*,103) nstep,i,j,
     .  '  exiting enloan   surflx increment =',amount/delt1,odhsi(i,j),
